...
void main() {
  runApp(MaterialApp(home: University(), debugShowCheckedModeBanner: false));
}
...
class University extends StatefulWidget {
  ...
  @override
  State<University> createState() => _AbroadLevelsState();
}

class _AbroadLevelsState extends State<University> {
  final List<Map<String, dynamic>> tileData = [
    {
      ...
      'content': [
        ...
        'LIVERPOOL JOHN MORRIS UNIVERSITY',
        ...
        'Regents University- United Kingdom',
    ...
    setState(() {
      filteredData = tileData.where((tile) {
        return tile['title'].toLowerCase().contains(searchTerm) ||
            (tile['content'] as List).any((item) =>
                (item as String).toLowerCase().contains(searchTerm));
      }).toList();
    });
  }
